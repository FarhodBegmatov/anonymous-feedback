...
import type { Faculty, RatingColors } from '@/types/Home';
...
export default function FeedbackForm({
    ...
    const facultiesToShow = useMemo<Faculty[]>(() => {
        if (!debouncedQuery) return faculties.data;

        const query = debouncedQuery.toLowerCase();

        return faculties.data.filter((faculty) => {
            const name = faculty.name[locale] || faculty.name.en || '';
            ...
            <div className="mx-auto mb-10 max-w-7xl px-4">
                <div className="grid grid-cols-1 gap-6 text-center sm:grid-cols-3">
                    ...
                    <StatCard
                        ...
                        title={
                            translations.total_feedbacks || 'Total Feedbacks'
                        }
                    ...
                    <StatCard
                        ...
                        title={
                            translations.global_average_grade ||
                            'Global Average'
                        }
                    ...
                    <>
                        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                            {facultiesToShow.map((faculty) => {
                                ...
                                const { ratingColor, barColor } =
                                    getRatingColors(rating);

                                return (
                                    <div
                                        key={faculty.id}
                                        className="flex h-full transform flex-col rounded-xl border bg-white shadow-sm transition hover:scale-[1.02] hover:shadow-xl"
                                    >
                                        {/* Header */}
